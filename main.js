...
function RenderLoop () {
  ...
  var ImageData = BackContextHandle.getImageData(
    0,
    0,
    CanvasWidth,
    CanvasHeight
  ...
  if (FrameRateTime >= FrameRateRefresh) {
    ...
    document.getElementById('FPSTextBox').value =
      FPS + ' / ' + 1 / TargetFrameTime
...
function RenderBackground () {
  ...
  for (
    var y = 0;
    y < Math.floor((CanvasHeight + SquareSize) / SquareSize);
    y++
  ) {
    for (
      var x = 0;
      x < Math.floor((CanvasWidth + SquareSize) / SquareSize);
      x++
    ) {
      ...
      if (x % 2 != y % 2) TargetColor.R = TargetColor.G = TargetColor.B = 235
