...
function RenderScene () {
  ...
  for (var i = 0; i < CubeVertex.length; i++) {
    ...
    WorkingVertex.z =
      -WorkingVertex.x * Math.sin(CameraRot.y) -
      WorkingVertex.z * Math.cos(CameraRot.y)
    WorkingVertex.x =
      -WorkingVertex.x * Math.cos(CameraRot.y) + Temp * Math.sin(CameraRot.y)

    Temp = WorkingVertex.z
    WorkingVertex.z =
      -WorkingVertex.y * Math.sin(CameraRot.x) +
      WorkingVertex.z * Math.cos(CameraRot.x)
    WorkingVertex.y =
      WorkingVertex.y * Math.cos(CameraRot.x) + Temp * Math.sin(CameraRot.x)

    Temp = WorkingVertex.x
    WorkingVertex.x =
      WorkingVertex.x * Math.cos(CameraRot.z) -
      WorkingVertex.y * Math.sin(CameraRot.z)
    WorkingVertex.y =
      WorkingVertex.y * Math.cos(CameraRot.z) + Temp * Math.sin(CameraRot.z)
  ...
  for (var i = 0; i < CubeFaces.length; i++) {
    ...
    var Color = {
      R: (CubeFaces[i].i * 50) % 255,
      G: (CubeFaces[i].i * 128) % 255,
      B: (CubeFaces[i].i * 200) % 255
    }
    ...
    var LengthAC = Math.sqrt(
      VectorAC.x * VectorAC.x +
        VectorAC.y * VectorAC.y +
        VectorAC.z * VectorAC.z
    )
    VectorAC = {
      x: VectorAC.x / LengthAC,
